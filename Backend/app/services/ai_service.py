...
async def generate_chat_answer(relevant_chunks: list[dict], question: str, current_article_title: str) -> str:
    ...
    for chunk in relevant_chunks:
        testo = chunk.get("chunk_text", "")
        # --- MODIFICA QUI: Usiamo il titolo invece dell'ID ---
        titolo = chunk.get("title", "Titolo Sconosciuto")
        if testo:
            context_text.append(f"--- Documento Titolo: {titolo} ---\n{testo}")
    ...
    prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            "Sei un assistente editoriale intelligente. L'utente sta attualmente leggendo l'articolo intitolato: '{current_article_title}'.\n\n"
            "Ho cercato nell'intero archivio e ho trovato questi frammenti correlati:\n\n"
            "CONTESTO GLOBALE:\n{context}\n\n"
            "REGOLE DI RISPOSTA:\n"
            "1. Rispondi basandoti ESCLUSIVAMENTE sulle informazioni presenti nel CONTESTO GLOBALE. Non inventare fatti, nomi o eventi non presenti nei frammenti.\n"
            "2. Se l'utente chiede spiegazioni sull'articolo in lettura, usa prioritariamente i frammenti con Titolo '{current_article_title}'.\n"
            "3. Se l'utente chiede di articoli simili o cosa altro c'è in archivio sul tema, guarda i frammenti con Titolo DIVERSO da '{current_article_title}'. "
            "Se e SOLO SE ne trovi di realmente pertinenti alla domanda, rispondi indicando il Titolo dell'articolo e un breve riassunto fedele al contenuto.\n"
            "4. Se nel CONTESTO GLOBALE non ci sono informazioni pertinenti, dillo chiaramente: "
            "'Mi dispiace, non ho trovato informazioni pertinenti nell'archivio.'\n"
            "5. Cita sempre il 'Documento Titolo' quando riporti un'informazione specifica.\n"
